...
public class ElectricCalc extends SherlockActivity {
...
//			setContentView(R.layout.unit_calc_720);
	...
	private void initControls(){
		...
		ArrayAdapter<CharSequence> adapter = ArrayAdapter.createFromResource(this, R.array.unit_electric, R.layout.my_spinner_text);
			...
			if(result==0){
				...
				if(!selected_type_from.equals(selected_type_to)){
					...
					for(int i=0;i<type_from.getCount();i++){
						...
						if (check.equalsIgnoreCase("ampere")){
							table.put(check, Double.valueOf(1));
						}
						if (check.equalsIgnoreCase("biot")){
							table.put(check, Double.valueOf(10));
						}
						if (check.equalsIgnoreCase("coulomb/second")){
							table.put(check, Double.valueOf(1));
						}if (check.equalsIgnoreCase("gilbert")){
							table.put(check, Double.valueOf(0.79577472));
						}
						if (check.equalsIgnoreCase("kiloampere")){
							table.put(check, Double.valueOf(1000));
						}
						if (check.equalsIgnoreCase("milliampere")){
							table.put(check, Double.valueOf(0.001));
						}
						if (check.equalsIgnoreCase("volt/ohm")){
							table.put(check, Double.valueOf(1));
						}
						if (check.equalsIgnoreCase("watt/volt")){
							table.put(check, Double.valueOf(1));
						}
					}
