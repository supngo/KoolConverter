...
public class EnergyCalc extends SherlockActivity {
...
//			setContentView(R.layout.unit_calc_720);
	...
	private void initControls(){
		...
		ArrayAdapter<CharSequence> adapter = ArrayAdapter.createFromResource(this, R.array.unit_energy, R.layout.my_spinner_text);
	    adapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
	    type_from.setAdapter(adapter);
	    type_from.setSelection(6);
			...
			if(result==0){
				...
				if(!selected_type_from.equals(selected_type_to)){
					...
					for(int i=0;i<type_from.getCount();i++){
						...
						if (check.equalsIgnoreCase("Btu(th)")){
							table.put(check, Double.valueOf(0.293015));
						}
						if (check.equalsIgnoreCase("calorie(th)")){
							table.put(check, Double.valueOf(0.001162));
						}
						if (check.equalsIgnoreCase("calorie(food)")){
							table.put(check, Double.valueOf(1.162778));
						}
						if (check.equalsIgnoreCase("foot-pound force")){
							table.put(check, Double.valueOf(0.000377));
						}
						if (check.equalsIgnoreCase("horsepower hour")){
							table.put(check, Double.valueOf(745.6998));
						}
						if (check.equalsIgnoreCase("inch-pound force")){
							table.put(check, Double.valueOf(0.000031));
						}
						if (check.equalsIgnoreCase("joule")){
							table.put(check, Double.valueOf(0.000274));
						}
						if (check.equalsIgnoreCase("kilocalorie(th)")){
							table.put(check, Double.valueOf(1.146416));
						}
						if (check.equalsIgnoreCase("kilogram-force meter")){
							table.put(check, Double.valueOf(0.002687));
						}
						if (check.equalsIgnoreCase("kilojoule")){
							table.put(check, Double.valueOf(0.273998));
						}
						if (check.equalsIgnoreCase("kilowatt hour")){
							table.put(check, Double.valueOf(1000));
						}
						if (check.equalsIgnoreCase("newton meter")){
							table.put(check, Double.valueOf(0.000274));
						}
						if (check.equalsIgnoreCase("watt hour")){
