...
public class DensityCalc extends SherlockActivity {
...
//			setContentView(R.layout.unit_calc_720);
	...
	private void initControls(){
		...
		ArrayAdapter<CharSequence> adapter = ArrayAdapter.createFromResource(this, R.array.unit_density, R.layout.my_spinner_text);
			...
			if(result==0){
				...
				if(!selected_type_from.equals(selected_type_to)){
					...
					for(int i=0;i<type_from.getCount();i++){
						...
						if (check.equalsIgnoreCase("grain/gallon")){
							kg_l = (double)0.453592/(7000*3.785412);
							table.put(check, Double.valueOf(kg_l));
						}
						if (check.equalsIgnoreCase("gram/cubic centimeter")){
							table.put(check, Double.valueOf(1));
						}
						if (check.equalsIgnoreCase("gram/liter")){
							kg_l = (double)1/1000;
							table.put(check, Double.valueOf(kg_l));
						}
						if (check.equalsIgnoreCase("gram/milliliter")){
							table.put(check, Double.valueOf(1));
						}
						if (check.equalsIgnoreCase("kilogram/cubic meter")){
							kg_l = (double)1/1000;
							table.put(check, Double.valueOf(kg_l));
						}
						if (check.equalsIgnoreCase("kilogram/liter")){
							table.put(check, Double.valueOf(1));
						}
						if (check.equalsIgnoreCase("milligram/milliliter")){
							kg_l = (double)1/1000;
							table.put(check, Double.valueOf(kg_l));
						}
						if (check.equalsIgnoreCase("milligram/liter")){
							kg_l = (double)1/1000000;
							table.put(check, Double.valueOf(kg_l));
						}
						if (check.equalsIgnoreCase("ounce/cubic inch")){
							kg_l = (double)0.453592 * 1000/(Math.pow(2.54, 3)*16);
							table.put(check, Double.valueOf(kg_l));
						}
						if (check.equalsIgnoreCase("ounce/gallon")){
							kg_l = (double)0.453592 /(3.785412*16);
							table.put(check, Double.valueOf(kg_l));
						}
						if (check.equalsIgnoreCase("pound/cubic inch")){
							kg_l = (double)0.453592 * 1000/(Math.pow(2.54, 3));
							table.put(check, Double.valueOf(kg_l));
						}
						if (check.equalsIgnoreCase("pound/cubic foot")){
							kg_l = (double)0.453592 * 1000/(Math.pow(30.48, 3));
							table.put(check, Double.valueOf(kg_l));
						}
						if (check.equalsIgnoreCase("pound/gallon")){
							kg_l = (double)0.453592/3.785412;
							table.put(check, Double.valueOf(kg_l));
						}
					}
